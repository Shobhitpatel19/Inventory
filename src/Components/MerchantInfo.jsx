...
const MarchentInfo = () => {
  ...
  const handleSwiggyAccordian = () => {
    setShowSwgy(!shoeSwgy);
  };
  const handleUPIAccordian = () => {
    setShowUPI(!showUPI);
  };
    ...
    if (selectedId) {
      axios
        .put(baseURL + "/fbs/locations", {
          locData: {
            _id: selectedId,
            loc_name: JSON.stringify({ name: marchentName, upid: marchentUPI }),
            user_id: userId,
            title: "UPI",
          },
        })
  ...
  const handleEdit = (marchentId) => {
    ...
    const mrDetils =
      marchentDtails.length &&
      marchentDtails.filter((mrList) => mrList._id === marchentId);
  ...
  const handleDelete = (deleteId) => {
    ...
    // axios.delete(baseURL + "/fbs/locations/" + deleteItemId).then((response) => {
    //   console.log(response.data);
    //   axios.get(`${getMarDetails}`).then((response) => {
    //     console.log(response.data);
    //     setMarchentDetails(response.data);
  ...
  const handleConfirmDelete = () => {
    ...
    axios
      .delete(baseURL + "/fbs/locations/" + deleteItemId)
      .then((response) => {
        console.log(response.data);
        axios.get(`${getMarDetails}`).then((response) => {
          console.log(response.data);
  ...
  return (
    <div className="container bg-light">
      ...
      <div className="m-2">
        <div
          className="fw-bold-5 text-center text-white rounded-2  bg-secondary w-75  accordian"
          onClick={handleUPIAccordian}
        >
          {" "}
          UPI Details{" "}
          <span>
          ...
          <div>
            <form style={{ padding: "10px" }} onSubmit={handleSubmit}>
              <div style={{ display: "flex" }}>
                <div>
                  <lable>
                    Marchent Name<span className="text-danger">*</span>
                  </lable>
                  <input
                    type="text"
                ...
                <div className="mx-5">
                  <lable>
                    Enter UPI adress<span className="text-danger">*</span>
                  </lable>
                  <input
                    type="text"
                    className="form-control w-100"
                ...
                <button className="btn btn-xs btn-success mt-3" type="submit">
                  save
                </button>
              </div>
            </form>
                      ...
                      <tr>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => handleEdit(upiDetalis._id)}
                          >
                            Edit
                          </button>
                        </td>
                        <td>
                          <button
                            ...
                            onClick={() => handleDelete(upiDetalis._id)}
                          >
                            Delete
                          </button>
                        </td>
      ...
      <div className="m-2">
        <div
          className="fw-bold-5 text-center text-white rounded-2 accordian   bg-secondary  w-75"
          onClick={handleSwiggyAccordian}
        >
          swiggy{" "}
          <span>
            {shoeSwgy ? (
      ...
      <div className="mx-2">
        <div
          className="fw-bold text-center  rounded-2 accordian  text-white bg-secondary w-75  "
          onClick={handleZomataAccordian}
        >
          zomato{" "}
          <span>
            {showZmta ? (
